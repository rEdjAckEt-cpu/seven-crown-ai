...
# ==========================================
# 📊 3. 競走馬データベース (構文・データ完全修正済)
# ==========================================
@st.cache_data
def get_infinity_horse_parameters(horse_names):
    # パラメーター構造: [キレ, 持続力, 旋回性能, 操縦性, パワー] (各100点満点)
    base_data = {
        "コスモキュランダ": [45, 88, 92, 70, 85],
        "フォーエバーヤング": [60, 95, 80, 90, 98],
        "ダノンデサイル": [75, 82, 85, 88, 80],
        "メイショウタバル": [50, 85, 80, 60, 95],
        "ブローザホーン": [70, 96, 85, 75, 92],
        "ベラジオオペラ": [80, 84, 90, 85, 82],
        "ファウストラーゼン": [55, 86, 75, 65, 80],
        "マスカレードボール": [92, 80, 75, 82, 72],
        "ジュウリョクピエロ": [65, 88, 82, 78, 88],
        "リアライズシリウス": [70, 78, 85, 80, 70],
        "スターアニス": [95, 75, 70, 85, 68],
        "アロヒアリイ": [52, 80, 82, 75, 78],
        "ミステリーウェイ": [50, 78, 75, 70, 76]
    }
    ...
    for name in horse_names:
        ...
        if name in base_data:
            ...
            # データベース未登録の下位層や新規馬に対する自動力学補正
            if "アース" in name or "インパクト" in name or "キレ" in name:
                final_data[name] = [85, 75, 70, 75, 65]
            elif "テラ" in name or "キング" in name or "スタミナ" in name:
                final_data[name] = [55, 85, 80, 70, 82]
            else:
                final_data[name] = [70, 75, 75, 75, 70]
    return final_data
...
input_horses_raw = st.text_area(
    "出走馬の名前をカンマ（、または ,）で区切って入力してください（下位クラス〜海外名馬まで対応）",
